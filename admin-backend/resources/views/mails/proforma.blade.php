    ...
    <div class="wrapper">
        <div class="wrap-div">
            ...
            <table cellpadding="0" cellspacing="0" style="width:100%; background:#fff; border:1px solid #d6d6d6;">
                <tr>
                    <td colspan="3"
                        style="background-color:#fff; border-bottom:1px dashed #d6d6d6; padding:10px 0px 12px 10px; margin:0px 0px 0px 0px; text-align:center;">
                        <h1 style="margin:0px; padding:0px; color:#000000; font-weight:normal;">
                            {{ $order->company->company_name }}</h1>
                        ...
                        <div style="font-size:12px; margin:5px 0px 0px 0px;">
                            ...
                            <span>
                                ...
                                @if (!empty($order->executive))
                                    <b>Billed By : </b>
                                    {{ $order->executive->first_name . ' ' . $order->executive->last_name }}
                                @endif
                            </span><br />
                ...
                <tr>
                    <td colspan="3" style="padding:10px 20px 0px 20px;">
                        <table width="100%"
                            style="font-size:13px; padding:5px 10px;  border-radius:3px; -moz-border-radius:3px;">
                            <tr>
                                <th align="left" style="border-bottom:1px dashed #ccc; line-height:30px;">Description
                                </th>
                                <th align="center" style="border-bottom:1px dashed #ccc; line-height:30px;">QTY</th>
                                <th align="center" style="border-bottom:1px dashed #ccc; line-height:30px;">Unit Rate
                                </th>
                            ...
                            @foreach ($order->items as $item)
                                <tr>
                                    @if ($item->discount_id > 0)
                                        <td style="line-height:20px; width:50%">
                                            {{ $item->product->product_name }}<br><span
                                                style="text-align:right">Discount @
                                                {{ $item->discount->details->discount_value }} %</span></td>
                                    @endif
                                    <td style="line-height:20px; width:50%">{{ $item->product->product_name }}</td>
                                    <td style="line-height:20px;width=10%" align="center">{{ $item->product_quantity }}
                                    </td>
                                    <td align="center" style="line-height:20px;" align="center;width=20%">
                                        {{ number_format($item->sale_amount / $item->product_quantity, 2, '.', ',') }}
                                    </td>
                                    <td align="center" style="line-height:20px;" align="center;width=20%">
                                        {{ number_format($item->sale_amount, 2, '.', ',') }}</td>
                                </tr>
                                @php
                ...
                <tr>
                    <td colspan="3" style="padding:10px 20px 0px 20px; border-bottom:1px dashed #ccc;">
                        <table width="100%" style="font-size:13px; padding:5px 0px;">
                            ...
                            <tr>
                                <td align="right" style="line-height:20px; width:80%">Item Total:</th>
                                <td align="center" style="line-height:20px;">
                                    {{ number_format($totalSaleAmount, 2, '.', ',') }}</th>
                            </tr>
                            @if ($order->discount_id > 0)
                                <tr>
                                    <td align="right" style="line-height:20px; width:80%">Order Level Savings :
                                        {{ $order->order_level_discount->discount_value }} %</th>
                                    <td align="center" style="line-height:20px;">
                            ...
                            @if ($order->charge_amount > 0)
                                @foreach ($order->miscellaneous as $charge)
                                    <tr>
                                        <td align="right" style="line-height:20px; width:80%">Service Charge @ 5 % :
                                            </th>
                                        <td align="center" style="line-height:20px;">
                                            {{ number_format($order->charge_amount, 2, '.', ',') }}</th>
                                    </tr>
                                @endforeach
                            @endif
                ...
                <tr style="" colspan="3">
                    ...
                    <td align="center">All values are in INR</td>
                </tr>
                    ...
                    <td valign="top" colspan="3">
                        <table cellpadding="0" cellspacing="0" width="100%">
                            <tr>
                                <td>
                                    <p
                                        style="font-family:calibri;color:#fff; font-size:12px; text-align:center; margin:0px; padding:5px 70px 5px 70px; border-top:1px dotted #d6d6d6;">
                                        This email is powered by <a href="https://www.bizwy.com" target="_blank"
                                            style="text-decoration:none; font-weight:bold;"><span
                                                style="color:#ffff">BizWy</span></a><br />
                                    <p></p>
                                </td>
                            </tr>
                        </table>
